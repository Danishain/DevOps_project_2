...
def Day_Night_Result_table(df, buy_morning=True, open_time=open_time, close_time=close_time,
    ...
    full_trades = pd.concat([open_trades, close_trades], ignore_index=True).sort_values(
        by=['timestamp', 'mispar_hoze']).reset_index(drop=True)
...
if st.button("Save Results"):
    ...
    s3.upload_file(f"diff{delta}open{open_time}close{close_time}.csv", "strategy-result-1",
                   f"diff{delta}open{open_time}close{close_time}.csv")
